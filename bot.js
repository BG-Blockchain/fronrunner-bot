...
const buyToken = async(account,tokenContract,gasPrice)=>{
  ...
  if (receipt && receipt.blockNumber && receipt.status === 1) { // 0 - failed, 1 - success
    console.log(`[BNB] -> [${config.whitelist[tokenContract]}] Transaction https://bscscan.com/tx/${receipt.transactionHash} mined, status success`);
  } else if (receipt && receipt.blockNumber && receipt.status === 0) {
    console.log(`[BNB] -> [${config.whitelist[tokenContract]}] Transaction https://bscscan.com/tx/${receipt.transactionHash} mined, status failed`);
  } else {
    console.log(`[BNB] -> [${config.whitelist[tokenContract]}] Transaction https://bscscan.com/tx/${receipt.transactionHash} not mined`);
  }
}
const sellToken = async(account,tokenContract,gasLimit,gasPrice,value=99)=>{
  const slippage = slippage_sell
  ...
  if (receipt_approve && receipt_approve.blockNumber && receipt_approve.status === 1) { 
    console.log(`[${config.whitelist[tokenContract]}] Approved https://bscscan.com/tx/${receipt_approve.transactionHash}`);
    const swap_txn = await contract.swapExactTokensForETH(
    ...
    if (receipt && receipt.blockNumber && receipt.status === 1) { // 0 - failed, 1 - success
      console.log(`[${config.whitelist[tokenContract]}] -> [BNB] Transaction https://bscscan.com/tx/${receipt.transactionHash} mined, status success`);
    } else if (receipt && receipt.blockNumber && receipt.status === 0) {
      console.log(`[${config.whitelist[tokenContract]}] -> [BNB] Transaction https://bscscan.com/tx/${receipt.transactionHash} mined, status failed`);
    } else {
      console.log(`[${config.whitelist[tokenContract]}] -> [BNB] Transaction https://bscscan.com/tx/${receipt.transactionHash} not mined`);
    }
  }
}
